...
allocates memory for every opaque handle it returns. That memory is **not**
managed by the .NET garbage collector.

## Rule: always call FreeObj

Every `IntPtr` returned by the following methods **must** be freed when no
longer needed:
...
`Blsct.EncodeAddress` returns a managed `string`. It internally frees both the
native return-value struct and the inner string pointer before returning; the
caller does **not** call `FreeObj` on the return value.
